...
/* Texto corrido preso em medida legivel; componentes usam a largura toda.
   Linha muito longa cansa a leitura, entao alargar o container nao pode
   arrastar o paragrafo junto. Tabelas, cards, figuras e vereditos ficam livres. */
...
/* JORNADA: faixas verticais lado a lado, lidas da esquerda para a direita
   como um percurso. A escala de cinza vai clareando para escurecendo, o que
   ja indica avanco sem precisar de cor. Texto escuro sobre fundo claro:
   contraste acima de 10:1 em todas as faixas. */
.jornada{display:flex; align-items:stretch; border-radius:18px; overflow:hidden; box-shadow:var(--sombra); margin:2rem 0 2.8rem; border:1px solid var(--borda)}

.jPasso{position:relative; flex:1 1 0; min-width:0; padding:1.25rem 1.05rem 1.35rem; color:var(--tinta-suave); display:flex; flex-direction:column; background:var(--marca); border-right:1px solid rgba(0,0,0,.06)}
.jPasso:last-child{border-right:none}
.jPasso[data-t="1"]{--marca:#F4F1EC; --realce:#7A6E64}
.jPasso[data-t="2"]{--marca:#EDE8E1; --realce:#6F6358}
.jPasso[data-t="3"]{--marca:#E5DED5; --realce:#65594E}
.jPasso[data-t="4"]{--marca:#DCD4C9; --realce:#5B4F44}
.jPasso[data-t="5"]{--marca:#D3C9BC; --realce:#51453A}
.jPasso[data-t="6"]{--marca:#ECE6F3; --realce:var(--acento-forte)}

.jSeta{display:none}

.jEtapa{display:none}
.jNum{position:relative; z-index:2; font-family:var(--fonte-dados); font-size:1.5rem; font-weight:700; line-height:1; letter-spacing:-.02em; margin-bottom:.5rem; display:block; color:var(--realce)}
.jTopo{position:relative; z-index:2; font-family:var(--fonte-titulo); font-size:1.06rem; font-weight:700; line-height:1.25; margin-bottom:.65rem; color:var(--tinta); min-height:2.5em; display:flex; align-items:flex-start}

/* Altura FIXA (nao minima): so assim a lista de apoio comeca na mesma linha
   em todas as faixas, independente do tamanho do texto do destaque. */
.jHero{position:relative; z-index:2; background:rgba(255,255,255,.7); border:1px solid rgba(0,0,0,.07); border-radius:9px; padding:.6rem .65rem; margin-bottom:.85rem; height:6.4em; display:flex; flex-direction:column; justify-content:center}
.jHeroNum{font-family:var(--fonte-dados); font-size:1.02rem; font-weight:700; line-height:1.3; letter-spacing:-.01em; color:var(--realce)}
.jHeroNum .jLinha{display:block}
/* Tabelinha de antes/depois por horizonte, para o destaque nao mentir dizendo
   um numero so quando o ganho varia com a distancia da previsao. */
.jPorH{display:grid; grid-template-columns:auto auto auto auto; gap:.1rem .4rem; font-family:var(--fonte-dados); font-size:.66rem; margin-top:.25rem; align-items:baseline}
.jPorH .jH{color:var(--muted)}
.jPorH .jDe{color:var(--muted)}
.jPorH .jPara{color:var(--realce); font-weight:700}
.jPorH .jSeta2{color:var(--faint)}
.jHeroNum small{font-size:.72rem; font-weight:600; color:var(--muted); letter-spacing:0}
.jHeroRot{font-size:.68rem; color:var(--muted); margin-top:.25rem; line-height:1.35}

.jCorpo{position:relative; z-index:2; font-size:.78rem; line-height:1.5; flex:1}
.jBloco{margin-bottom:.7rem}
.jBloco:last-child{margin-bottom:0}
.jRot{display:block; font-size:.6rem; text-transform:uppercase; letter-spacing:.09em; font-weight:700; color:var(--faint); margin-bottom:.28rem}
.jCorpo ul{margin:0; padding:0; list-style:none}
.jCorpo ul li{position:relative; padding-left:.7rem; margin:.25rem 0}
.jCorpo ul li::before{content:''; position:absolute; left:0; top:.55em; width:4px; height:4px; border-radius:50%; background:var(--realce); opacity:.7}
.jCorpo b{font-weight:700; color:var(--tinta)}
.jMetrica{display:block; font-family:var(--fonte-dados); font-size:.68rem; background:rgba(255,255,255,.7); border:1px solid rgba(0,0,0,.07); border-radius:5px; padding:.16rem .4rem; margin:.2rem 0}

@media (max-width:900px){
  .jornada{flex-direction:column}
  .jPasso{border-right:none; border-bottom:1px solid rgba(0,0,0,.06)}
}

/* FICHA DO MODELO: especificacao com rotulo a esquerda e valor a direita.
   Os parametros ficam um por linha, e nao amontoados numa frase so. */
.fichaModelo{background:var(--superficie); border:1px solid var(--borda); border-radius:var(--raio); box-shadow:var(--sombra); overflow:hidden; margin:1.4rem 0}
.fmTitulo{background:var(--acento-suave); border-bottom:1px solid var(--borda); padding:.85rem 1.1rem; font-weight:700; color:var(--acento-forte); font-size:.95rem}
.fmLinha{display:grid; grid-template-columns:minmax(120px,180px) 1fr; gap:1rem; padding:.7rem 1.1rem; border-bottom:1px solid var(--linha-grade); align-items:baseline}
.fmLinha:last-child{border-bottom:none}
.fmRot{font-size:.74rem; text-transform:uppercase; letter-spacing:.05em; color:var(--muted); font-weight:700}
.fmVal{color:var(--tinta-suave); font-size:.9rem; line-height:1.5}
.fmVal b{color:var(--tinta)}
.fmParams{display:flex; flex-direction:column; gap:.32rem}
.fmParam{display:flex; align-items:baseline; gap:.6rem; font-family:var(--fonte-dados); font-size:.8rem}
.fmParam .fmChave{color:var(--tinta); background:var(--elevado); border:1px solid var(--borda); border-radius:6px; padding:.1rem .45rem}
.fmParam .fmIgual{color:var(--faint)}
.fmParam .fmNum{color:var(--acento-forte); font-weight:700}
@media (max-width:640px){.fmLinha{grid-template-columns:1fr; gap:.25rem}}

/* CARDS: grade de cartoes com faixa colorida no topo, por tom semantico. */
.cards{display:grid; grid-template-columns:repeat(auto-fit,minmax(250px,1fr)); gap:1.1rem; margin:1.6rem 0}
.card{background:var(--superficie); border:1px solid var(--borda); border-radius:var(--raio-p); padding:1rem 1.1rem; box-shadow:var(--sombra); border-top:3px solid var(--borda-forte)}
.card.bom{border-top-color:var(--bom)}
.card.atencao{border-top-color:var(--atencao)}
.card.critico{border-top-color:var(--critico)}
.card.acento{border-top-color:var(--acento-forte)}
.cardRot{font-size:.72rem; text-transform:uppercase; letter-spacing:.05em; color:var(--muted); font-weight:700; margin-bottom:.45rem}
.cardNum{font-family:var(--fonte-dados); font-size:1.65rem; font-weight:700; color:var(--tinta); line-height:1.1; margin-bottom:.35rem}
.card.bom .cardNum{color:var(--bom)}
.card.critico .cardNum{color:var(--critico)}
.card.acento .cardNum{color:var(--acento-forte)}
.cardTxt{font-size:.83rem; color:var(--tinta-suave); line-height:1.5}
.cardTxt b{color:var(--tinta)}

/* VEREDITO: caixa de conclusao, para separar o que e afirmacao do resto. */
.veredito{border-left:4px solid var(--acento-forte); background:var(--acento-suave); border-radius:0 var(--raio-p) var(--raio-p) 0; padding:.95rem 1.15rem; margin:1.4rem 0}
.veredito.atencao{border-left-color:var(--atencao); background:color-mix(in srgb, var(--atencao) 10%, var(--superficie))}
.veredito.critico{border-left-color:var(--critico); background:color-mix(in srgb, var(--critico) 10%, var(--superficie))}
.veredito .vRot{font-size:.72rem; text-transform:uppercase; letter-spacing:.05em; font-weight:700; color:var(--acento-forte); display:block; margin-bottom:.3rem}
.veredito.atencao .vRot{color:var(--atencao)}
.veredito.critico .vRot{color:var(--critico)}
.veredito p{margin:0; font-size:.9rem; color:var(--tinta-suave); line-height:1.55}
.veredito b{color:var(--tinta)}
