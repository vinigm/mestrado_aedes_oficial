...
def selecionar_treino_do_regime(
    ...
    Args:
        ...
        horizonte: Quantas semanas a frente se preve. So entram linhas
            cuja resposta ja tinha acontecido na data de corte.
    ...
    passado = dados_validos.loc[
        dados_validos[COLUNA_DATA] + pd.Timedelta(weeks=horizonte) <= data_de_corte
    ]
...
def executar_ablacao(dados: pd.DataFrame, horizonte: int) -> pd.DataFrame:
    ...
    for data_de_corte in semanas_de_avaliacao:
        ...
        for regime in REGIMES:
            treino = selecionar_treino_do_regime(
                dados_validos, data_de_corte, regime, horizonte
            )
