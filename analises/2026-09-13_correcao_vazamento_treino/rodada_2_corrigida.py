...
CAMINHO_INFODENGUE = Path(
    "/Users/viniciusguerra/Library/CloudStorage/GoogleDrive-vinigm@gmail.com/"
    "Meu Drive/Mestrado/Pesquisa/Meu_Projeto/modelagem_aedes/dados/entradas/"
    "infodengue_poa/infodengue_poa_dengue.csv"
)
PASTA_SAIDA = Path(__file__).resolve().parent / "saidas"
...
def carregar_tabela_final(caminho_arquivo: Path, alvo: str = "confirmados") -> pd.DataFrame:
    """Lê a tabela_final.csv do projeto e a deixa em ordem cronológica.

    Diferença em relação à versão de 16/08/2026: aceita escolher o alvo. Com
    'confirmados' o comportamento é IDÊNTICO ao original (casos do SINAN);
    com 'notificados' a coluna 'casos' passa a ser a série do InfoDengue,
    casada pela data de início da semana. Nesse caso a coluna de confirmados
    é removida, porque ela é quase uma função do notificado da mesma semana e
    não pode sobrar por perto quando o alvo é o notificado.

    Args:
        caminho_arquivo: Caminho do CSV original do projeto (só leitura).
        alvo: 'confirmados' (SINAN, série desde 2018) ou 'notificados'
            (InfoDengue, série desde 2010).

    Returns:
        Cópia da tabela com a coluna de data renomeada para 'data' e a
        coluna do alvo escolhido renomeada para 'casos' (mesmo nome que
        o pipeline do projeto usa), ordenada por data.

    Raises:
        FileNotFoundError: Se algum arquivo não existir no caminho informado.
        ValueError: Se o alvo pedido não for 'confirmados' nem 'notificados'.
    """
    if not caminho_arquivo.exists():
        raise FileNotFoundError(f"Tabela final não encontrada em: {caminho_arquivo}")

    if alvo not in ("confirmados", "notificados"):
        raise ValueError(f"alvo deve ser 'confirmados' ou 'notificados'; recebido: {alvo!r}")

    tabela = pd.read_csv(caminho_arquivo, parse_dates=["data_inicio_semana_epidemi"])
    tabela = tabela.rename(columns={"data_inicio_semana_epidemi": "data"})

    if alvo == "confirmados":
        tabela = tabela.rename(columns={"casos_confirmados": COLUNA_CASOS})
        return tabela.sort_values("data").reset_index(drop=True)

    if not CAMINHO_INFODENGUE.exists():
        raise FileNotFoundError(f"InfoDengue não encontrado em: {CAMINHO_INFODENGUE}")

    tabela = tabela.drop(columns=["casos_confirmados"])
    infodengue = pd.read_csv(CAMINHO_INFODENGUE, parse_dates=["data_iniSE"])
    casos_notificados = infodengue[["data_iniSE", "casos"]].rename(
        columns={"data_iniSE": "data", "casos": COLUNA_CASOS}
    )
    tabela = tabela.merge(casos_notificados, on="data", how="left")
    return tabela.sort_values("data").reset_index(drop=True)
...
def executar_walk_forward(
    ...
    for indice_corte in range(
        ...
        # CORRECAO 13/09/2026: so entra no treino a linha cuja RESPOSTA ja tinha
        # acontecido nesta data. Ver motor/corte_temporal.py e o README desta pasta.
        treino = dados_validos.loc[
            dados_validos["data"] + pd.Timedelta(weeks=horizonte)
            <= teste["data"].to_numpy()[0]
        ]
...
def main() -> None:
    ...
    analisador = argparse.ArgumentParser(
        description="Teste A de equivalencia clima x vetor, na serie longa de clima (2012+)."
    )
    analisador.add_argument(
        "--alvo",
        default="confirmados",
        choices=["confirmados", "notificados"],
        help="qual serie de casos o modelo tenta prever",
    )
    argumentos = analisador.parse_args()
    alvo_escolhido = argumentos.alvo

    PASTA_SAIDA.mkdir(parents=True, exist_ok=True)

    tabela_final = carregar_tabela_final(CAMINHO_TABELA_FINAL, alvo_escolhido)
    ...
    semanas_com_alvo = int(tabela_com_features[COLUNA_CASOS].notna().sum())
    print(f"ALVO: casos {alvo_escolhido} | {semanas_com_alvo} semanas com alvo nao-nulo")
    ...
    for horizonte in HORIZONTES:
        ...
        caminho_persistencia = PASTA_SAIDA / f"rodada_2_{alvo_escolhido}_previsoes_persistencia_h{horizonte}.csv"
        ...
        for conjunto in conjuntos_de_features:
            ...
            caminho_previsoes = PASTA_SAIDA / f"rodada_2_{alvo_escolhido}_previsoes_{conjunto.nome}_h{horizonte}.csv"
    ...
    caminho_resultados = PASTA_SAIDA / f"rodada_2_{alvo_escolhido}_resultados_equivalencia.csv"
